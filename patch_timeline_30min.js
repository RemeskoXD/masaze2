...
const newComponent = `function PublicTimelinePicker({ date, time, endTime, setTime, setEndTime, setDisplayEndTime, serviceId, selectedAddons, reservations, specificDates, openingHours }: any) {
    ...
    const handleManualTimeChange = (e: any) => {
        ...
        if (!val) return;
        let mins = parseTime(val);
        // Snap manual entry to nearest 30 mins
        mins = Math.round(mins / 30) * 30;
        checkAndSetTime(mins);
    };

    // Split timeline into chunks of 3 hours for rows
    const chunks: { start: number; end: number }[] = [];
    const step = 3 * 60; // 3 hours per block
        ...
        <div className="mt-4 border border-gold/20 rounded-xl p-5 bg-white/50 shadow-sm flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                <div className="flex flex-col gap-1 w-full sm:w-auto relative">
                    ...
                    <input 
                        ...
                        className="bg-white border border-gold/30 rounded-lg px-4 py-2 text-text-dark focus:border-gold outline-none w-full sm:w-40 font-mono shadow-sm"
                    />
                </div>
            ...
            <div className="flex flex-col gap-10 mt-2">
                {chunks.map((chunk, idx) => {
                    ...
                    // Generate ticks for every 30 mins
                    const ticks = [];
                    for (let t = chunk.start; t <= chunk.end; t += 30) {
                        ticks.push(t);
                    }
                        ...
                        <div key={idx} className="relative flex flex-col">
                            ...
                            <div 
                                ...
                                onClick={(e) => {
                                    ...
                                    clickedMins = Math.round(clickedMins / 30) * 30; // Snap to 30 mins
                            ...
                            >
                                ...
                                {/* Minor grid lines (every 30 mins) */}
                                {ticks.map(tick => {
                                    ...
                                    return (
                                        <div 
                                            key={tick}
                                            className={\`absolute top-0 bottom-0 border-l \${tick % 60 === 0 ? 'border-green-300/60' : 'border-green-200/40'} z-0\`}
                                            style={{ left: \`\${left}%\` }}
                                        />
                                    );
                                })}
...
console.log('patched timeline 30 mins');
